...
int main (int argc, char * const * argv)
{
	...
	description.add_options ()
		...
		("debug_profile_process", "Profile active blocks processing (only for nanobitcoin_dev_network)")
		("debug_profile_votes", "Profile votes processing (only for nanobitcoin_dev_network)")
		("debug_profile_frontiers_confirmation", "Profile frontiers confirmation speed (only for nanobitcoin_dev_network)")
	...
	if (ec == nano::error_cli::unknown_command)
	{
		...
		{
			if (nano::network_constants::active_network != nano::networks::nanobitcoin_dev_network)
			{
				...
				struct mismatched_t
				{
					...
					std::string get_entry () const
					{
						...
						% rep.to_account () % hardcoded.format_balance (nano::Mbtc_ratio, 0, true) % ledger.format_balance (nano::Mbtc_ratio, 0, true) % diff.format_balance (nano::Mbtc_ratio, 0, true));
					}
				};
				...
				auto const outlier_threshold = std::max (nano::Gbtc_ratio, mismatch_mean.number () + 1 * mismatch_stddev.number ());
				...
				auto const newcomer_threshold = std::max (nano::Gbtc_ratio, mismatch_mean.number ());
				...
				auto newcomer_entry = [] (auto const & rep) {
					return boost::str (boost::format ("representative %1% hardcoded --- ledger %2%") % rep.first.to_account () % nano::uint128_union (rep.second).format_balance (nano::Mbtc_ratio, 0, true));
				};

				std::cout << boost::str (boost::format ("hardcoded weight %1% Mnano at %2% blocks\nledger weight %3% Mnano at %4% blocks\nmismatched\n\tsamples %5%\n\ttotal %6% Mnano\n\tmean %7% Mnano\n\tsigma %8% Mnano\n")
				% total_hardcoded.format_balance (nano::Mbtc_ratio, 0, true)
				% hardcoded_height
				% total_ledger.format_balance (nano::Mbtc_ratio, 0, true)
				% ledger_height
				% mismatched.size ()
				% mismatch_total.format_balance (nano::Mbtc_ratio, 0, true)
				% mismatch_mean.format_balance (nano::Mbtc_ratio, 0, true)
				% mismatch_stddev.format_balance (nano::Mbtc_ratio, 0, true));
				...
				// Log more data
				auto const log_threshold = nano::Gbtc_ratio;
