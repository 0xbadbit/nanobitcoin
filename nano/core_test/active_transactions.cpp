...
TEST (active_transactions, inactive_votes_cache_existing_vote)
{
	...
	auto send = builder.send ()
				.previous (latest)
				.destination (key.pub)
				.balance (nano::dev::constants.genesis_amount - 100 * nano::Gbtc_ratio)
				.sign (nano::dev::genesis_key.prv, nano::dev::genesis_key.pub)
				.work (*system.work.generate (latest))
				.build_shared ();
...
TEST (active_transactions, inactive_votes_cache_election_start)
{
	...
	auto amount = ((node.online_reps.trended () / 100) * node.config.hinted_scheduler.hinting_threshold_percent) / 2 + 1000 * nano::Gbtc_ratio;
...
TEST (active_transactions, vote_replays)
{
	...
	ASSERT_EQ (nano::vote_code::replay, node.active.vote (vote_open1));
	ASSERT_EQ (nano::Gbtc_ratio, node.ledger.weight (key.pub));
...
TEST (active_transactions, fork_replacement_tally)
{
	...
	auto send_last = builder.make_block ()
					 ...
					 .previous (latest)
					 .representative (nano::dev::genesis_key.pub)
					 .balance (balance - 2 * nano::Gbtc_ratio)
					 .link (key.pub)
					 .sign (nano::dev::genesis_key.prv, nano::dev::genesis_key.pub)
					 .work (*system.work.generate (latest))
					 .build_shared ();
	...
	for (auto i (0); i < reps_count; i++)
	{
		auto fork = builder.make_block ()
					...
					.previous (latest)
					.representative (nano::dev::genesis_key.pub)
					.balance (balance - nano::Gbtc_ratio - i)
					.link (key.pub)
					.sign (nano::dev::genesis_key.prv, nano::dev::genesis_key.pub)
					.work (*system.work.generate (latest))
					.build_shared ();
...
TEST (active_transactions, limit_vote_hinted_elections)
{
	...
	// Enough weight to trigger election hinting but not enough to confirm block on its own
	const auto amount = ((node.online_reps.trended () / 100) * node.config.hinted_scheduler.hinting_threshold_percent) + 1000 * nano::Gbtc_ratio;
