...
{
	nano::test::system system (1);
	auto price1 (system.nodes[0]->price (nano::Gbtc_ratio, 1));
	ASSERT_EQ (nano::node::price_max * 100.0, price1);
	auto price2 (system.nodes[0]->price (nano::Gbtc_ratio * int (nano::node::free_cutoff + 1), 1));
	ASSERT_EQ (0, price2);
	auto price3 (system.nodes[0]->price (nano::Gbtc_ratio * int (nano::node::free_cutoff + 2) / 2, 1));
	ASSERT_EQ (nano::node::price_max * 100.0 / 2, price3);
	auto price4 (system.nodes[0]->price (nano::Gbtc_ratio * int (nano::node::free_cutoff) * 2, 1));
	ASSERT_EQ (0, price4);
}
...
TEST (node, DISABLED_fork_stale)
{
	...
	auto send3 = builder.make_block ()
				 ...
				 .previous (nano::dev::genesis->hash ())
				 .representative (nano::dev::genesis_key.pub)
				 .balance (nano::dev::constants.genesis_amount - nano::Mbtc_ratio)
				 .link (key1.pub)
				 .sign (nano::dev::genesis_key.prv, nano::dev::genesis_key.pub)
				 .work (0)
				 .build_shared ();
	...
	auto send1 = builder.make_block ()
				 ...
				 .previous (send3->hash ())
				 .representative (nano::dev::genesis_key.pub)
				 .balance (nano::dev::constants.genesis_amount - 2 * nano::Mbtc_ratio)
				 .link (key1.pub)
				 .sign (nano::dev::genesis_key.prv, nano::dev::genesis_key.pub)
				 .work (0)
				 .build_shared ();
...
TEST (node, unconfirmed_send)
{
	...
	auto send1 = wallet1->send_action (nano::dev::genesis->account (), key2.pub, 2 * nano::Mbtc_ratio);
	...
	ASSERT_EQ (node2.balance (key2.pub), 2 * nano::Mbtc_ratio);
	...
	auto send3 = wallet2->send_action (key2.pub, nano::dev::genesis->account (), nano::Mbtc_ratio);
...
TEST (node, rep_list)
{
	...
	wallet0->send_action (nano::dev::genesis_key.pub, key1.pub, nano::Mbtc_ratio);
...
TEST (node, no_voting)
{
	...
	// Broadcast a confirm so others should know this is a rep node
	wallet1->send_action (nano::dev::genesis_key.pub, key1.pub, nano::Mbtc_ratio);
...
TEST (node, online_reps_election)
{
	...
	ASSERT_EQ (0, node1.online_reps.online ());
	node1.vote_processor.vote_blocking (vote, std::make_shared<nano::transport::fake::channel> (node1));
	ASSERT_EQ (nano::dev::constants.genesis_amount - nano::Gbtc_ratio, node1.online_reps.online ());
}
...
TEST (node, confirm_quorum)
{
	...
	nano::amount new_balance = node1.online_reps.delta () - nano::Gbtc_ratio;
...
TEST (node, block_processor_signatures)
{
	...
	auto send4 = builder.make_block ()
				 ...
				 .previous (send3->hash ())
				 .representative (nano::dev::genesis_key.pub)
				 .balance (nano::dev::constants.genesis_amount - 4 * nano::Gbtc_ratio)
				 .link (key3.pub)
				 .sign (nano::dev::genesis_key.prv, nano::dev::genesis_key.pub)
	...
	auto send5 = builder.make_block ()
				 ...
				 .previous (send3->hash ())
				 .representative (nano::dev::genesis_key.pub)
				 .balance (nano::dev::constants.genesis_amount - 5 * nano::Gbtc_ratio)
				 .link (key3.pub)
				 .sign (nano::dev::genesis_key.prv, nano::dev::genesis_key.pub)
				 .work (*node1.work_generate_blocking (send3->hash ()))
				 .build_shared ();
