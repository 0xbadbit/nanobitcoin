...
TEST (wallet, send_race)
{
	...
	for (auto i (1); i < 60; ++i)
	{
		ASSERT_NE (nullptr, system.wallet (0)->send_action (nano::dev::genesis_key.pub, key2.pub, nano::Gbtc_ratio));
		ASSERT_EQ (nano::dev::constants.genesis_amount - nano::Gbtc_ratio * i, system.nodes[0]->balance (nano::dev::genesis_key.pub));
	}
}
