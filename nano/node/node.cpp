	...
	if (!init_error ())
	{
		...
		if ((network_params.network.is_live_network () || network_params.network.is_beta_network ()) && !flags.inactive_node)
		{
			...
			if (use_bootstrap_weight)
			{
				...
				{
					logger.always_log ("Using bootstrap rep weight: ", rep.first.to_account (), " -> ", nano::uint128_union (rep.second).format_balance (Mbtc_ratio, 0, true), " XRB");
				}
			}
...
{
	debug_assert (balance_a >= amount_a * nano::Gbtc_ratio);
	auto balance_l (balance_a);
	double result (0.0);
	for (auto i (0); i < amount_a; ++i)
	{
		balance_l -= nano::Gbtc_ratio;
		auto balance_scaled ((balance_l / nano::Mbtc_ratio).convert_to<double> ());
