...
usage() {
	printf "Usage:\n"
	printf "  $0 nanobitcoin_node [daemon] [cli_options] [-l] [-v size]\n"
	printf "    daemon\n"
	printf "      start as daemon\n\n"
	printf "    cli_options\n"
	printf "      nanobitcoin_node cli options <see nanobitcoin_node --help>\n\n"
	printf "    -l\n"
	...
	printf "      usage\n\n"
	printf "default:\n"
	printf "  $0 nanobitcoin_node daemon -l\n"
}
...
if [ "$1" = 'nanobitcoin_node' ]; then
	command="${command}nanobitcoin_node"
	shift
	for i in $@; do
		case $i in
		"daemon")
...
raidir="${HOME}/Nanobitcoin${dirSuffix}"
nanodir="${HOME}/NanobitcoinData${dirSuffix}"
